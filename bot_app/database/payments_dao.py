...
async def create_payment(
    ...
    Всегда сериализуем meta → JSON-строка (совместимо и с TEXT, и с JSONB колонкой).
    """
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            """,
            user_hash, provider, inv, label, int(amount_rub), currency, status, meta_json,
        )
    return inv
...
async def get_payment(invoice_id: str) -> Optional[Dict[str, Any]]:
    ...
    if not row:
        return None
    d = dict(row)
    # meta мог сохраниться как TEXT — приведём к dict
    m = d.get("meta")
    if isinstance(m, str):
        try:
            d["meta"] = json.loads(m)
...
async def get_user_payments(user_id: int, limit: int = 20) -> List[Dict[str, Any]]:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT id, provider, invoice_id, label, amount_rub, currency, status, meta, created_at, paid_at
            FROM payments
            WHERE user_hash=$1
